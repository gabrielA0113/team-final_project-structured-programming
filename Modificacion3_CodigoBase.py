...
def mostrar_configuracion_sistema():
    """Recorre con un bucle FOR el diccionario configuracion_sistema y muestra su contenido."""
    print(f"\n{CIAN}--- CONFIGURACIÓN DEL SISTEMA ---{RESET}")
    for clave, valor in configuracion_sistema.items():
        print(f"  {clave}: {valor}")
    print(f"{CIAN}----------------------------------{RESET}")
...
# 4. Se agregó la función mostrar_configuracion_sistema(), que usa un bucle FOR (estructura de control) para recorrer configuracion_sistema.items().
# 5. Se agregó la opción "4. Ver configuración del sistema" al menú principal y su rama "elif opcion_elegida == 4" que invoca la nueva función.
# 6. Se mantuvo intacto el resto del código existente.